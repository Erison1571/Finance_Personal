...
interface IncomeDialogProps {
  open: boolean;
  onClose: () => void;
  onSave: (income: IncomeFormData) => void;
  income?: Income;
  isEdit: boolean;
}
...
export const IncomeDialog: React.FC<IncomeDialogProps> = ({
  ...
  useEffect(() => {
    if (income && isEdit) {
      setFormData({
        ...
        value: income.value,
        datePrevista: income.datePrevista,
        dateEfetiva: income.dateEfetiva || '',
        obs: income.obs || '',
        isMensal: false,
        quantidadeMeses: 1
      });
      setValueInput(formatBRL(income.value));
    } else {
      setFormData({
        categoryId: '',
        typeId: '',
  ...
  const handleSwitchChange = (field: keyof IncomeFormData, checked: boolean) => {
    setFormData(prev => ({ 
      ...prev, 
      [field]: checked,
            ...
            {formData.isMensal && (
              <Box mt={2}>
                ...
                <Alert severity="info" sx={{ mt: 1 }}>
                  Serão criadas {formData.quantidadeMeses} receitas mensais, uma para cada mês.
                </Alert>
              </Box>
            )}
          </Box>
